...
export const getDecisionOutcomes = async (
  req: AuthRequest,
  res: Response,
): Promise<void> => {
  try {
    ...
    if (!decisionId) {
      res
        .status(400)
        .json({ error: "Missing required query parameter: decision_id" });
      return;
    }
...
export const getOutcome = async (
  ...
  try {
    ...
    const decisionRecords = await db
      ...
      .from(decisions)
      .where(
        and(eq(decisions.id, outcome.decisionId), eq(decisions.userId, userId)),
      )
      .limit(1);
  ...
  try {
    ...
    if (!decisionId || satisfactionScore === undefined || !actualResults) {
      res
        ...
        .json({
          error:
            "decisionId, satisfactionScore, and actualResults are required",
        });
      return;
    }
    ...
    const timeElapsedDays = Math.floor(
      (Date.now() - new Date(decision.decisionDate).getTime()) /
        (1000 * 60 * 60 * 24),
    );
    ...
    // ── Async: Intelligence hooks (fire-and-forget) ──
    // 1. Recompute profile with new outcome data
    computeUserProfile(userId).catch((err) =>
      console.error("[CreateOutcome] Profile recomputation failed:", err),
    );

    // 2. Extract memories from outcome reflections/lessons
    const memoryContent = [
      lessonsLearned ? `Lesson learned: ${lessonsLearned}` : "",
      reflections ? `Reflection: ${reflections}` : "",
      surprises ? `Surprise outcome: ${surprises}` : "",
      contextChanges ? `Context change: ${contextChanges}` : "",
      `Decision: "${decision.title}" (${decision.category}). Satisfaction: ${satisfactionScore}/10. Would decide again: ${wouldDecideAgain ?? "unknown"}.`,
    ]
      .filter(Boolean)
      .join("\n");
    ...
    extractAndStoreMemories(
      ...
      console.error("[CreateOutcome] Memory extraction failed:", err),
    );

    // 3. Detect patterns after new data point
    detectPatterns(userId).catch((err) =>
      console.error("[CreateOutcome] Pattern detection failed:", err),
    );
  } catch (error) {
...
export const updateOutcome = async (
  ...
  try {
    ...
    const decisionRecords = await db
      ...
      .where(
        and(
          eq(decisions.id, existing.decisionId),
...
export const getPendingCheckins = async (
  ...
  try {
    ...
    const pending = await db
      ...
      .where(
        and(
          eq(outcomeReminders.userId, userId),
          eq(outcomeReminders.status, "pending"),
        ),
      );
...
// ─── Schedule Checkin ───────────────────────────────────────────────────────
export const scheduleCheckin = async (
  req: AuthRequest,
  res: Response,
): Promise<void> => {
  try {
    ...
    if (!decisionId || !scheduledDate) {
      res
        .status(400)
        .json({ error: "decisionId and scheduledDate are required" });
      return;
    }
...
export const completeCheckin = async (
  ...
  try {
    ...
    const updated = await db
      ...
      .where(
        and(eq(outcomeReminders.id, id), eq(outcomeReminders.userId, userId)),
      )
      .returning();
...
// ─── Skip Checkin ───────────────────────────────────────────────────────────
export const skipCheckin = async (
  req: AuthRequest,
  res: Response,
): Promise<void> => {
  try {
